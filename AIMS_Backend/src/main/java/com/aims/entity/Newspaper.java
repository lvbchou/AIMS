...
import jakarta.persistence.*; import lombok.*; import java.time.LocalDate; import java.util.*;
@Entity @Table(name="newspaper") @PrimaryKeyJoinColumn(name="product_id")
@Getter @Setter @NoArgsConstructor
public class Newspaper extends PrintableProduct {
    @Column(name="editor_in_chief") private String editorInChief;
    @Column(name="issue_number") private String issueNumber;
    @Column(name="publication_frequency") private String publicationFrequency;
    @Column(name="issn") private String ISSN;
    @ElementCollection
    @CollectionTable(name="newspaper_section",schema="aims",joinColumns=@JoinColumn(name="product_id"))
    @Column(name="section_name") private List<String> sections=new ArrayList<>();
    public Newspaper(String title,String category,String barcode,String image,
        ...
        this.editorInChief=editorInChief; this.issueNumber=issueNumber;
        this.publicationFrequency=publicationFrequency; this.ISSN=ISSN;
